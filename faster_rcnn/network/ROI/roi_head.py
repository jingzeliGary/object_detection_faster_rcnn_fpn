...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    Arguments:
        class_logits : 预测类别概率信息，Tensor(nums_proposals, num_classes)
        box_regression : 预测边目标界框回归信息, Tensor(nums_proposals, num_classes * 4)
        labels : 真实类别信息, List(Tensor(nums_proposals))
        regression_targets : 真实目标边界框信息, List(nums_anchors_all_feature_maps, 4)
...
class RoIHeads(torch.nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        Args:
            proposals: # List(Tensor(post_nms_top_n + nums_gt_boxes, 4))
            gt_boxes: # List(Tensor(nums_gt_boxes, 4))
            gt_labels: # List(Tensor(nums_gt_boxes))
    ...
    def select_training_samples(self,
        ...
        list元素个数为batch_size
        Args:
            proposals: rpn预测的boxes, List(Tensor(post_nms_top_n, 4))
            targets: 标注信息， List(Dict)
        """
        '''
        # 检查target数据是否为空
