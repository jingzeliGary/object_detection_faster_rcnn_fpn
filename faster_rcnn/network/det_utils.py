...
def encode_boxes(reference_boxes, proposals, weights):
    ...
    proposals_x1 = proposals[:, 0].unsqueeze(1)  # Tensor(nums_anchor_batch,1)
    proposals_y1 = proposals[:, 1].unsqueeze(1)
    proposals_x2 = proposals[:, 2].unsqueeze(1)
    proposals_y2 = proposals[:, 3].unsqueeze(1)

    reference_boxes_x1 = reference_boxes[:, 0].unsqueeze(1)  # Tensor(nums_anchor_batch,1)
    ...
    return targets  # Tensor(nums_anchor_batch,4)
...
class BoxCoder(object):
    ...
    def encode(self, reference_boxes, proposals):
        ...
        '''
        gt boxes, anchors 计算 box_regs
        Args:
            reference_boxes: 每张图片 anchor 对应的 gt_box信息, List(Tensor(nums_anchors_gt, 4))
            proposals:每张图片 anchor/proposal 信息, List(Tensor(anchors_all_feature_maps, 4))
        '''
        # 统计每张图像的anchors个数，方便后面拼接在一起处理后在分开
        # reference_boxes和proposal数据结构相同
        boxes_per_image = [len(b) for b in reference_boxes]
        reference_boxes = torch.cat(reference_boxes, dim=0)  # Tensor(nums_anchor_batch, 4)
        proposals = torch.cat(proposals, dim=0)   # Tensor(nums_anchor_batch, 4)

        # targets_dx, targets_dy, targets_dw, targets_dh
        targets = self.encode_single(reference_boxes, proposals)
        return targets.split(boxes_per_image, 0)  # List(nums_anchors_all_feature_maps, 4)
    ...
    def decode(self, rel_codes, boxes):
        ...
        '''
        基于 预测的回归参数 计算 proposal
        Args:
            rel_codes: 预测的回归参数, Tensor(batch_size * num_anchors_all_feature_maps, 4)
            boxes: batch中每张图片生成的anchors坐标， List(num_anchors_all_feature_maps, 4)
        '''
