    ...
    def forward(self, x):
        ...
        for name, module in self.items():
            ...
            if name in self.return_layers:
                ...
                out[out_name] = x
        return out  # OrderDict('layer1': feature_map)
...
class BackboneWithFPN(nn.Module):
    ...
    def __init__(self,
                 ...
                 return_layers=None,  # 输出的层
                 in_channels_list=None, # 输出层的in_channels
                 out_channels=256,  # 通过fpn后得到的每个特征层的channel
                 extra_blocks=None,  # 在输出的特征层基础上额外添加的层结构
                 re_getter=True):  # 返回 OrderDict('layer': feature_map)
        super().__init__()
